...
2. Read the original skill under `../../skills/<name>/SKILL.md`.
...
| `${CLAUDE_PLUGIN_ROOT}` | Resolve to the repository root. |
...
1. Read the relevant file under `../../agents/`.
...
- Otherwise use the repo's `scripts/qa-browse` CLI through `exec_command`.
...
- Put Codex-specific behavior in new files under `./codex-skills/`, `./.codex-plugin/`, or new Codex-only state paths under `./.workflow-adapter/codex/`.
