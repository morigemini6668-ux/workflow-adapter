...
export default async function agents(args: string[]): Promise<void> {
  ...
  const data = response.data as {
    name: string; cli: string; role: string; state: string;
    current_task: string | null; pane_id: string;
  }[];
  ...
  console.log('Name'.padEnd(20) + 'CLI'.padEnd(8) + 'Role'.padEnd(14) + 'State'.padEnd(10) + 'Task');
