...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: sequential check list, inherently branchy
export default async function doctor(_args: string[]): Promise<void> {
  ...
  if (tmuxPath) {
    ...
    if (version && compareSemver(version, TMUX_MIN_VERSION) >= 0) {
      ...
      checks.push({
        name: "tmux",
        ok: false,
        detail: `version ${version ?? "unknown"} < ${TMUX_MIN_VERSION} required`,
      });
  ...
  if (claudePath) {
    ...
    checks.push({
      name: "claude",
      ok: false,
      detail: "not found (optional — needed for Claude workers)",
    });
  }
  ...
  if (codexPath) {
    ...
    checks.push({
      name: "codex",
      ok: false,
      detail: "not found (optional — needed for Codex workers)",
    });
  }
  ...
  if (existsSync(UNIFLOW_SOCKETS_DIR)) {
    ...
    checks.push({
      name: "sockets dir",
      ok: false,
      detail: "not found — will be created on first start",
    });
  }
  ...
  if (existsSync(claudePluginJson)) {
    try {
      ...
      integrations.push({
        ...
        ok: false,
        detail: "plugin.json is invalid JSON",
      });
    }
  } else {
    integrations.push({
      name: "Claude Code plugin",
      ok: false,
      detail: 'not installed \u2014 run "uniflow install"',
    });
  }
    ...
    try {
      ...
      if (files.length > 0) {
        integrations.push({
          ...
          ok: true,
          detail: `${files.length} commands found`,
        });
      } else {
        integrations.push({
          name: "Claude plugin commands",
          ok: false,
          detail: "no command files found",
        });
      }
    } catch {
      integrations.push({
        name: "Claude plugin commands",
        ok: false,
        detail: "cannot read commands directory",
      });
    }
  } else {
    integrations.push({
  ...
  if (uniflowPath) {
    ...
    integrations.push({
      name: "Global CLI",
      ok: false,
      detail: 'not in PATH \u2014 run "uniflow install" (includes bun link)',
    });
  }
