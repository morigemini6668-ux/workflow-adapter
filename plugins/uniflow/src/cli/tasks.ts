...
export default async function tasks(args: string[]): Promise<void> {
  ...
  const data = response.data as {
    ...
    assignee: string | null; priority: number; depends_on: string[];
  }[];
  ...
  console.log('ID'.padEnd(15) + 'P'.padEnd(4) + 'Status'.padEnd(14) + 'Assignee'.padEnd(16) + 'Subject');
  console.log('-'.repeat(80));
  for (const t of data) {
    const assignee = t.assignee ?? '-';
    console.log(
      t.id.padEnd(15) + `${t.priority}`.padEnd(4) + t.status.padEnd(14) + assignee.padEnd(16) + t.subject,
    );
  }
}
