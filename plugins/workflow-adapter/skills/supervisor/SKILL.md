...
프로젝트에 `.claude/` 디렉토리나 CLAUDE.md, AGENTS.md가 있으면 먼저 읽는다.
프로젝트 자체에 정의된 커스텀 스킬, 에이전트, 워크플로우가 있을 수 있고,
이들은 해당 프로젝트에 특화되어 있으므로 범용 플러그인 스킬보다 우선 고려한다.

확인 대상:
- `CLAUDE.md` / `AGENTS.md` — 프로젝트 컨벤션, 에이전트 정의
- `.claude/commands/` — 프로젝트 커스텀 커맨드
- `.claude/settings.json` — 프로젝트 레벨 설정

### 2-2. 전체 스킬 목록 확인

system-reminder에 로드된 모든 플러그인 스킬 목록에서 유저 요청과 관련된 스킬을 식별한다.
프로젝트 내 스킬과 플러그인 스킬이 겹치면 프로젝트 내 스킬을 우선한다.
