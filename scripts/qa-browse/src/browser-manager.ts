 ...
 *
 * Chromium crash → log error → process.exit(1)
...
export class BrowserManager {
  ...
  getPage(): Page {
    const page = this.pages.get(this.activeTabId);
    if (!page) throw new Error('No active page. Use "qa-browse goto <url>" first.');
    return page;
  }
  ...
  async resolveRef(selector: string): Promise<{ locator: Locator } | { selector: string }> {
    if (selector.startsWith('@e') || selector.startsWith('@c')) {
      ...
      if (count === 0) {
        throw new Error(
          `Ref ${selector} (${entry.role} "${entry.name}") is stale. Run 'snapshot' for fresh refs.`
        );
      }
  ...
  private wirePageEvents(page: Page) {
    page.on('framenavigated', (frame) => {
      if (frame === page.mainFrame()) this.clearRefs();
    });
    ...
    page.on('console', (msg) => {
      addConsoleEntry({ timestamp: Date.now(), level: msg.type(), text: msg.text() });
    });

    page.on('request', (req) => {
      addNetworkEntry({ timestamp: Date.now(), method: req.method(), url: req.url() });
    });
