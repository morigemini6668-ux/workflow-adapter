  ...
  switch (command) {
    ...
    case 'html': {
      ...
      if (selector) {
        ...
        if ('locator' in resolved) return await resolved.locator.innerHTML({ timeout: 5000 });
    ...
    case 'js': {
      const expr = args[0];
      if (!expr) throw new Error('Usage: qa-browse js <expression>');
      const result = await page.evaluate(wrapForEvaluate(expr));
      return typeof result === 'object' ? JSON.stringify(result, null, 2) : String(result ?? '');
    }

    case 'eval': {
      const filePath = args[0];
      if (!filePath) throw new Error('Usage: qa-browse eval <js-file>');
      validateReadPath(filePath);
      if (!fs.existsSync(filePath)) throw new Error(`File not found: ${filePath}`);
      const code = fs.readFileSync(filePath, 'utf-8');
      const result = await page.evaluate(wrapForEvaluate(code));
      return typeof result === 'object' ? JSON.stringify(result, null, 2) : String(result ?? '');
    }

    case 'css': {
      const [selector, property] = args;
      if (!selector || !property) throw new Error('Usage: qa-browse css <selector> <property>');
      const resolved = await bm.resolveRef(selector);
      if ('locator' in resolved) {
        return await resolved.locator.evaluate((el, prop) => getComputedStyle(el).getPropertyValue(prop), property);
      }
    ...
    case 'attrs': {
      const selector = args[0];
      if (!selector) throw new Error('Usage: qa-browse attrs <selector>');
      const resolved = await bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'console': {
      if (args[0] === '--clear') { consoleBuffer.clear(); return 'Console buffer cleared.'; }
    ...
    case 'network': {
      if (args[0] === '--clear') { networkBuffer.clear(); return 'Network buffer cleared.'; }
    ...
    case 'dialog': {
      if (args[0] === '--clear') { dialogBuffer.clear(); return 'Dialog buffer cleared.'; }
    ...
    case 'is': {
      ...
      if (!property || !selector) throw new Error('Usage: qa-browse is <property> <selector>');
      const resolved = await bm.resolveRef(selector);
      ...
      switch (property) {
        ...
        case 'focused':  return String(await locator.evaluate(el => el === document.activeElement));
        default: throw new Error(`Unknown property: ${property}. Use: visible, hidden, enabled, disabled, checked, editable, focused`);
      }
    }

    case 'cookies':
      return JSON.stringify(await page.context().cookies(), null, 2);

    case 'storage': {
      if (args[0] === 'set' && args[1]) {
        await page.evaluate(([k, v]) => localStorage.setItem(k, v), [args[1], args[2] || '']);
        return `Set localStorage["${args[1]}"]`;
      }
