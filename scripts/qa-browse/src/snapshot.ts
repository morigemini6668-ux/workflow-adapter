...
export const SNAPSHOT_FLAGS: Array<{
  ...
  { short: '-i', long: '--interactive', description: 'Interactive elements only with @e refs', optionKey: 'interactive' },
  ...
  { short: '-D', long: '--diff', description: 'Unified diff against previous snapshot', optionKey: 'diff' },
  { short: '-a', long: '--annotate', description: 'Annotated screenshot with ref labels', optionKey: 'annotate' },
  { short: '-o', long: '--output', description: 'Output path for annotated screenshot', takesValue: true, valueHint: '<path>', optionKey: 'outputPath' },
  { short: '-C', long: '--cursor-interactive', description: 'Cursor-interactive elements (@c refs)', optionKey: 'cursorInteractive' },
];
    ...
    try {
      ...
      await page.evaluate((boxes) => {
        for (const { ref, box } of boxes) {
          ...
          overlay.className = '__qa_browse_annotation__';
          ...
          label.style.cssText = 'position: absolute; top: -14px; left: 0; background: red; color: white; padding: 0 3px; font-size: 10px; font-weight: bold;';
      ...
      await page.evaluate(() => {
        document.querySelectorAll('.__qa_browse_annotation__').forEach(el => el.remove());
      });
