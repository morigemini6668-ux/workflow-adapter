...
If not in a worktree session but `worktree_path` exists on disk, inform the user: "Worktree at `{worktree_path}` exists but was not created by this session. Please clean up manually if needed."
